...
export const EVENT_TYPES = {
  ...
  export class GGSDK {
     ...
     */
    private checkRegisteredListenersAndAdd(eventType: string, callback: (e: MessageEvent) => void): void {
    ...
    public getGameData(defaultData: GameData, callback: GameDataCallback): void {
      ...
      const eventCallback = (e: MessageEvent) => {
        if (e.data.event_type === EVENT_TYPES.GG_SET_GAME_DATA) {
          clearTimeout(timeout);
          callback(e.data.payload.gameData);
        }
      };
